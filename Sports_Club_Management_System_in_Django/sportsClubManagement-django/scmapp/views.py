...
def search(request):
    ...
    bookings = Book_ground.objects.filter(
        Q(field__name__icontains=query) | 
        Q(event__name__icontains=query)
    )
    fields = Field.objects.filter(name__icontains=query)
    events = Event.objects.filter(name__icontains=query)
    context = {
        'Book_ground': Book_ground,
        'fields': fields,
        'Event': Event,
        'query': query
    }
    return render(request, 'booking_system/search.html', context) 
